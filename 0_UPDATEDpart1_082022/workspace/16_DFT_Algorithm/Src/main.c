...
float ReX_buffer[INPUT_SIGNAL_LEN_KHZ_1_15/2];
float ImX_buffer[INPUT_SIGNAL_LEN_KHZ_1_15/2];
//////////////////////////////////////////////////
...
/*Calculation DFT of the input signal*/
	dft_signal_calc((float32_t*)inputSignal_f32_1kHz_15kHz,
			(float32_t*)ReX_buffer,
			(float32_t*)ImX_buffer,
			(uint32_t) INPUT_SIGNAL_LEN_KHZ_1_15);

	get_dft_output_magnitude((float32_t*)ReX_buffer,
			(uint32_t) INPUT_SIGNAL_LEN_KHZ_1_15/2);
...
/******************************************************************/
	...
	while(1){
		// Watch on Arduino serial plotter or https://x-io.co.uk/serial-oscilloscope/
