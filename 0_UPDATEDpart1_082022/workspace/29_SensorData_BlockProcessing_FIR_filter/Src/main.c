...
/*This buffer_arr will be use by convolution*/
uint32_t g_output_signal_arr[INPUT_SIG_LEN + IMP_RESPONSE_1HZ_3HZ - 1];
...
/******************************************************************/
	...
	/*NOTE: if g_processFlag is set to 1 , then Do DSP processing*/
		...
		if(g_process_Flag){
			...
			ui32_convolution((uint32_t*) sensor_DataBuffer,
							 (float32_t*) lpf_1hz_3hz_impulse_response,
						     (uint32_t*) g_output_signal_arr,
						     (uint32_t) INPUT_SIG_LEN,
							 (uint32_t) IMPULSE_SIG_LENGTH);

			for(int i = 0; i < (INPUT_SIG_LEN + IMPULSE_SIG_LENGTH - 1); i++){
